...
public unsafe struct ULGPUState : IEquatable<ULGPUState>
{
	...
	public readonly Span<float> Scalar =>
		...
		MemoryMarshal.CreateSpan<float>(ref Unsafe.AsRef(in scalar_0), 8);
#else
...
#if NETCOREAPP2_1_OR_GREATER || NETSTANDARD2_1
		MemoryMarshal.CreateSpan<Vector4>(ref Unsafe.AsRef(in vector_0), 8);
#else
...
#if NETCOREAPP2_1_OR_GREATER || NETSTANDARD2_1
		MemoryMarshal.CreateSpan<Matrix4x4>(ref Unsafe.AsRef(in clip_0), 8)[.._ClipSize];
#else
