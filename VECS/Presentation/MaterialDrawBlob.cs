...
namespace VECS
{
    public interface IRenderBlob
    {
        ...
        public void Draw(RendererFrameInfo frameInfo,VkCommandBuffer commandBuffer, int pushConstantId);
    }
    ...
    public class MaterialDrawBlob
    {
        ...
        {
            TargetMaterialV2?.ExecuteDrawCommands(frameInfo,frameInfo.CommandBuffer,MatDrawCommands, MatDrawCount, indirectCmdBuffer);
        }

        public void Execute(RendererFrameInfo frameInfo, VkCommandBuffer commandBuffer, SwapChainBuffer<VkDrawIndexedIndirectCommand> indirectCmdBuffer)
        {
            TargetMaterialV2?.ExecuteDrawCommands(frameInfo, commandBuffer, MatDrawCommands, MatDrawCount, indirectCmdBuffer);
        }

        public void Execute(RendererFrameInfo frameInfo, VkCommandBuffer commandBuffer, int pushConstantId, SwapChainBuffer<VkDrawIndexedIndirectCommand> indirectCmdBuffer)
        {
    ...
    public class RenderBlob : IRenderBlob, IDisposable
    {
        ...
        private readonly MaterialDrawBlob _allInOneDrawBlob;

        public MaterialDrawBlob AllInOne => _allInOneDrawBlob;
        ...
        public unsafe RenderBlob(uint maxDraws)
        {
            ...
            _allInOneDrawBlob = new(null);
        }

        public void RebuildBlob(EntityManager entityManager, List<Entity> entities)
        {
            _drawCount = (uint)entities.Count;
            GenerateEarlyDraws(entityManager, entities, out var earlySort);
        ...
        private unsafe void GenerateEarlyDraws(EntityManager entityManager, List<Entity> entities, out Task mainSort)
        {
            ...
            if (_earlyDrawCommands.Length != _drawCount)
            {
                ...
                Array.Resize(ref _modelMatricesBuffer, (int)_drawCount);
            }
        ...
        private void BuildMaterialDrawCommands()
        {
            ...
            _allInOneDrawBlob.EarlyDrawCount = _drawCount;
            _allInOneDrawBlob.SetEarlyDrawOffset(0, _indexers);
            if (_allInOneDrawBlob.MatDrawCommands.Length < totalDraws)
            {
                Array.Resize(ref _allInOneDrawBlob.MatDrawCommands, totalDraws);
            }
            _allInOneDrawBlob.MatDrawCount = totalDraws;
            int offset = 0;
            for (int i = 0; i < _drawBlobs.Length; i++)
            {
                _drawBlobs[i].MatDrawCommands.CopyTo(_allInOneDrawBlob.MatDrawCommands, offset);
                offset += _drawBlobs[i].MatDrawCommands.Length;
            }

            Application.ParallelFor(totalDraws, (i) =>
            {
                var cmd = _allInOneDrawBlob.MatDrawCommands[i];
                cmd.Bloom = false;
                cmd.Variant = 0;
                cmd.Material = 0;
                cmd.Entity = 0;
                _allInOneDrawBlob.MatDrawCommands[i] = cmd;
            });

            // set mat draw commands
        ...
        public void Draw(RendererFrameInfo frameInfo, VkCommandBuffer commandBuffer,int pushConstantId)
        {
            for (int i = 0; i < _drawBlobs.Length; i++)
            {
                if (_drawBlobs[i].MatDrawCount > 0)
                {
                    _drawBlobs[i].Execute(frameInfo, commandBuffer, pushConstantId, _indirectCmdBuffer);
                }
            }
        }

        public void DrawSlice(RendererFrameInfo frameInfo, int sliceIndex, VkCommandBuffer commandBuffer)
        {
            var region = _drawSlices[sliceIndex];
            
            for (int i = region.StartIndex; i < region.Offset; i++)
            {
                _drawBlobs[_drawBlobMap[i]].Execute(frameInfo,commandBuffer, _indirectCmdBuffer);
            }
        }
