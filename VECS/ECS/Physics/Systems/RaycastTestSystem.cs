    ...
    public class RaycastTestSystem : SystemBase
    {
        ...
        public override void OnUpdate(EntityManager entityManager)
        {
            if (_cameraQuery.HasEntities)
            {
                ...
                var renderBounds = World.GetSystem<DebugDrawUtilities>();
                ...
                renderBounds.DrawLine(worldPoint, Vector3.Zero, new Vector4(1, 0, 1, 1).ToVkColor());

                bool input = InputManager.Instance.GetMouseButtonUp(0);
                rayCast.MaxDst = 100f;
                rayCast.Direction = Vector3.UnitZ;
                renderBounds.DrawLine(rayCast.Origin, rayCast.RayEnd, new Vector4(0, 0, 1, 1).ToVkColor());
                rayCast.Origin = new(-1, 4f, -20);
                renderBounds.DrawLine(rayCast.Origin, rayCast.RayEnd, new Vector4(0, 1, 0, 1).ToVkColor());
                if (input && World.Simulation.Raycast(rayCast, out RaycastHit hit))
                {
                    Console.WriteLine(hit.Collidable.BodyHandle.Value.ToString());
                }
                rayCast.Origin = new(1, 4f, -20);
                renderBounds.DrawLine(rayCast.Origin, rayCast.RayEnd, new Vector4(0, 1, 1, 1).ToVkColor());
                if (input && World.Simulation.Raycast(rayCast, out hit))
                {
                    Console.WriteLine(hit.Collidable.BodyHandle.Value.ToString());
                }
                rayCast.Origin = new(0, 1.5f, -20);
                renderBounds.DrawLine(rayCast.Origin, rayCast.RayEnd, new Vector4(1, 1, 0, 1).ToVkColor());
                if (input && World.Simulation.Raycast(rayCast, out hit))
                {
                    Console.WriteLine(hit.Collidable.BodyHandle.Value.ToString());
                }
